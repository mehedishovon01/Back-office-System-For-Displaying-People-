...
class SubsidiaryWiseLedgerReportService
{
    ...
    {
        $data['accounts'] = Account::query()->where('account_subsidiary_id', $request->account_subsidiary_id ?? -1)
        ...
        $data['accountTransactions'] = Account::query()->whereIn('id', $request->accounts ?? [])
