...
@section('page-header')
<i class="fa fa-list"></i> Sales Edit
@stop
@push('style')
...
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="widget-box widget-color-white ui-sortable-handle clearfix" id="widget-box-7">
            ...
            <!-- INPUTS -->
            <form action="{{ route('sales.update', $production_sale->id) }}" method="post">
                @csrf
                @method('PUT')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select required id="customer_id" name="customer_id" class="chosen-select-100-percent" data-placeholder="- Select Company Name -">
                                    ...
                                    <option value="{{ $key }}" {{$value->id == $production_sale->customer_id ? 'selected' : ''}}>{{ $name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="col-12">
                                ...
                                <table id="myTable" class="table table-bordered order-list">
                                    ...
                                    <tbody>
                                        @foreach($production_sale->productionSaleDetails as $details)
                                        <tr>
                                            <td class="count"></td>
                                            <td>
                                                <div class="col-sm-12 prod-price">
                                                    <select required name="product_id[]" onchange="enableQty('quantity-enable', 'unit', 'quantity', 'hide-unitid', 'price', this)" class="input-qty input-sm chosen-select-100-percent" data-placeholder="- Select Account -">
                                                        ...
                                                        <option value="{{ $data['id'] }}" {{$data['id'] == $details->product_id ? 'selected' : ''}} data-unit="{{ $data['unitName'] }}" data-unid="{{ $data['unitId'] }}" data-qty="{{ $data['qty'] }}" data-price="{{ $data['price'] }}">{{ $data['name'] }}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <td>
                                                <input name="available_quantity[]" value="{{ $details->product->opening_quantity }}" readonly required type="text" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right quantity" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td>
                                                <input required name="price[]" value="{{ $details->product->selling_price }}" type="text" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right quantity-enable price calculate-total" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td>
                                                <input required name="assign_qty[]" value="{{ $details->assign_qty }}" type="text" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right assign-qty quantity-enable calculate-qty calculate-total" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td>
                                                <input required name="subtotal[]" value="{{ $details->total_amount }}" readonly type="text" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right subtotal" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                            ...
                            <div class="col-md-6 pull-right">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-12 col-sm-8 @error('cost') has-error @enderror">
                                        <input type="number" required name="discount_amount" value="{{ $production_sale->discount_amount }}" class="discount calculate-total dicount-enable text-right form-control">
                                        @error('cost')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                            ...
                            <div class="col-md-6 pull-right">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-12 col-sm-8 @error('end_date') has-error @enderror">
                                        ...
                                        <input type="number" name="previous_due" value="{{ $production_sale->previous_due }}" required class="total-credit text-right form-control">
                                        @error('end_date')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 pull-right">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-12 col-sm-8 @error('end_date') has-error @enderror">
                                        <input type="number" name="total_amount" value="{{ $production_sale->total_amount }}" required class="totalAmount text-right form-control">
                                        @error('end_date')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 pull-right">
                                <div class="form-group">
                                    ...
                                    <div class="col-xs-12 col-sm-8 @error('end_date') has-error @enderror">
                                        <input type="number" name="payable_amount" value="{{ $production_sale->payable_amount }}" required class="payableAmount total-credit text-right form-control">
                                        @error('end_date')
                                        <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
