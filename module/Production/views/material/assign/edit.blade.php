...
@section('page-header')
<i class="fa fa-list"></i> Raw Materials Assign Edit
@stop
@push('style')
...
<div class="row">
    <div class="col-sm-12">
        ...
        <div class="widget-box widget-color-white ui-sortable-handle clearfix" id="widget-box-7">
            ...
            <!-- INPUTS -->
            <form action="{{ route('materials-assign.update', $data->id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="row" style="width: 100%; margin: 0 0 20px !important;">
                    <div class="col-sm-12 px-4">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input required name="date" class="form-control date-picker" id="id-date-picker-1" type="text" value="{{ $data->date }}" data-date-format="yyyy-mm-dd">
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select required id="factory_id" name="factory_id" class="chosen-select-100-percent" data-placeholder="- Select Factory Name -">
                                    ...
                                    <option value="{{ $factory->id }}" {{ $data->factory_id == $factory->id ? 'selected' : '' }}>{{ $factory->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-12">
                                <table id="myTable" class="table table-bordered order-list">
                                    ...
                                    <tbody>
                                        @foreach($data->materialDetails as $details)
                                        <tr>
                                            <td class="count"></td>
                                            <td>
                                                <div class="col-sm-12 prod-price">
                                                    <select required name="purchase_id[]" onchange="enableQty('quantity-enable', 'unit', 'quantity', 'hide-unitid', this)" class="input-qty input-sm chosen-select-100-percent" data-placeholder="- Select Account -">
                                                        <option></option>
                                                        @foreach($products as $value)
                                                        <option value="{{ $value->id }}" {{ $details->product_id == $value->product_id ? 'selected' : '' }} data-unit="{{ $value->product->unit->name }}" data-unid="{{ $value->product->unit->id }}" data-qty="{{ $value->quantity }}">{{ $value->product->name }}</option>
                                                        @endforeach
                                                    </select>
                                            ...
                                            <td>
                                                <input name="unit[]" value="{{ $details->product->unit->id }}" class="hide-unitid" type="hidden">
                                                <input readonly required type="text" value="{{ $details->product->unit->name }}" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right unit" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td>
                                                ...
                                                @foreach($products as $value)
                                                @if($details->product_id == $value->product_id)
                                                value="{{ $value->quantity }}"
                                                @endif
                                                @endforeach
                                                readonly required type="text" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right quantity" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td>
                                                <input required name="assign_qty[]" value="{{ $details->assign_qty }}" type="text" onkeypress="return checkOnlyNumber(event)" class="form-control input-sm text-right quantity-enable calculate-qty" />
                                                @error('credit')
                                                <span class="text-danger"> {{ $message }}</span>
                                                @enderror
                                            </td>
                        ...
                        <td>
                            <div class="col-sm-12 prod-price">
                                <select required name="purchase_id[]" onchange="enableQty('quantity-enable', 'unit', 'quantity', 'hide-unitid', this)" class="input-qty input-sm chosen-select-100-percent" data-placeholder="- Select Account -">
                                    <option></option>
                                    @foreach($products as $value)
                                    <option value="{{ $value->id }}" data-unit="{{ $value->product->unit->name }}" data-unid="{{ $value->product->unit->id }}" data-qty="{{ $value->quantity }}">{{ $value->product->name }}</option>
                                    @endforeach
                                </select>
